...
const dataEndpoint = (router) => {
    ...
    router.get('/api/post/:id', async (request, response, next) => {
        response.status(200).send({posts: posts[request.params.id]});
    });
    // New endpoint for updating a singular post
    router.put('/api/post/:id', async (request, response, next) => {
        const postId = request.params.id+1;
        ...
        if (!postToUpdate) {
            ...
            postToUpdate.title = request.body.title || postToUpdate.title;
            postToUpdate.text = request.body.text || postToUpdate.text;
            postToUpdate.image = request.body.image || postToUpdate.image;

            response.status(200).send({ post: postToUpdate });
        }
    });
};
